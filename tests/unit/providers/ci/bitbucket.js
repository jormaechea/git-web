		...
		describe('Bitbucket', () => {
			...
			it('Should return the CI list url', () => {
				bitbucketRemoteUrls.forEach(remoteUrl => {
					...
					assert.deepStrictEqual(bitbucket.mapRemoteToUrl(remoteUrlData, {}), 'https://bitbucket.org/my-workspace/my-repo/pipelines');
				});
			});
			...
			it('Should return the CI execution url', () => {
				bitbucketRemoteUrls.forEach(remoteUrl => {
					...
					assert.deepStrictEqual(bitbucket.mapRemoteToUrl(remoteUrlData, {
						executionId: '455'
					}), 'https://bitbucket.org/my-workspace/my-repo/pipelines/results/455');
				});
			});
